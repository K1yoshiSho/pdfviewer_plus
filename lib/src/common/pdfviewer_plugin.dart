...
import '../platform_interface/pdfviewer_platform_interface.dart';
...
class PdfViewerPlugin {
  ...
  Future<int> initializePdfRenderer(Uint8List documentBytes) async {
    ...
    final String? pageCount = await PdfViewerPlatform.instance.initializePdfRenderer(documentBytes, _documentID!);
  ...
  Future<List<dynamic>?> getPagesHeight() async {
    _originalHeight = await PdfViewerPlatform.instance.getPagesHeight(_documentID!);
    return _originalHeight;
  }

  /// Retrieves original width of PDF pages.
  Future<List<dynamic>?> getPagesWidth() async {
    _originalWidth = await PdfViewerPlatform.instance.getPagesWidth(_documentID!);
  ...
  bool _hasRenderedImage(int pageIndex, double scale, bool considerScale) {
    ...
    _renderingPages?.forEach((RenderedImage image) {
      if ((image.pageIndex == pageIndex && !considerScale) ||
          (image.pageIndex == pageIndex && considerScale && image.scale == scale)) {
        hasImage = true;
      }
    });
    return hasImage;
  ...
  Future<Map<int, List<dynamic>>?> _getImage(
      int pageIndex, double currentScale, bool isZoomChanged, int currentPageNumber) async {
    if (_renderingPages != null &&
        (!_hasRenderedImage(pageIndex, currentScale, false) ||
            (isZoomChanged && pageIndex == currentPageNumber && !_hasRenderedImage(pageIndex, currentScale, true)))) {
      final RenderedImage renderedImage = RenderedImage(pageIndex, currentScale);
      _renderingPages!.add(renderedImage);
      _nativeImage = CancelableOperation<Uint8List?>.fromFuture(
          PdfViewerPlatform.instance.getImage(pageIndex, currentScale, _documentID!));
      ...
      if (!kIsDesktop) {
        ...
        imageFuture = imageFuture.timeout(const Duration(milliseconds: 3000), onTimeout: () {
          _renderingPages?.remove(renderedImage);
          return null;
        });
        imageFuture = image;
  ...
  Future<Map<int, List<dynamic>>?> getSpecificPages(int startPageIndex, int endPageIndex, double currentScale,
      bool isZoomChanged, int currentPageNumber, bool canRenderImage) async {
    imageCache.clear();
    if (!canRenderImage) {
      _nativeImage?.cancel();
    ...
    for (int pageIndex = startPageIndex; pageIndex <= endPageIndex; pageIndex++) {
      if (_renderedPages != null &&
          (!_renderedPages!.containsKey(pageIndex) ||
              (isZoomChanged && pageIndex == currentPageNumber && currentScale > 1.75))) {
        currentScale = (pageIndex == currentPageNumber) ? currentScale : 1.0;
        if (_pageScale != null && (!_pageScale!.containsKey(pageIndex) || _pageScale![pageIndex] != currentScale)) {
          _pageScale![pageIndex] = currentScale;
          await _getImage(pageIndex, currentScale, isZoomChanged, currentPageNumber);
        }
      }
    }
